...
let ShopOrderList = React.createClass({
    getInitialState() {
        this.data = []
    ...
    getData() {
        ...
        //通过外部传入的参数来决定显示的类型
        fetch('/lflweb/shop/getShopOrder/' + custid + '/' + this.state.pageNum + '/' + this.props.type).then(response => response.json()).then(items => {
    ...
    linkTo(rowData) {
        this.props.dispatch({
            type: 'sOrderDet/getAddress',
            payload: rowData
        });
        location.href = `???#/shopOrderDetail/${rowData.orderid}`
    },
    ...
    renderListViewRow(rowData) {
        ...
        return (
            <div className={styles1.orderItem}>
